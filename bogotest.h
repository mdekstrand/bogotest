...
typedef struct _BTFixtureInfo {
    BTestSetupFunc setup;
    BTestTeardownFunc teardown;
    BTestFunc *funcs;
} BTFixtureInfo;
...
typedef struct _BTSuiteInfo {
    const char *name;
    SBTFixtureInfo *fixtures;
} BTSuiteInfo;
#define BT_SUITE_INFO_NULL { NULL, NULL }

int _bt_assert(int expr, const char *file, int line, const char *msg, ...);
...
void _bt_assert_doubles_equal(double act, double exp, double delta,(act, exp)(act, exp)
        const char *file, int line, const char *expr);
